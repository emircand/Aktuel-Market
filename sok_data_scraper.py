...
# Load URLs from file
with open('sok_category_href.txt', 'r') as file:
...
def extract_element_info(category_name):
    try:
        ...
        product_name_element = driver.find_element(By.XPATH, "//h1[@class='ProductMainInfoArea_productName__bKRVD']")
        ...
        try:
            current_price_element = driver.find_element(By.XPATH, "//span[@class='CPriceBox-module_discountedPrice__15Ffw']")
        except NoSuchElementException:
            current_price_element = driver.find_element(By.XPATH, "//span[@class='CPriceBox-module_price__bYk-c']")
        current_price = current_price_element.text.strip()
        
        # Extract old price (nullable)
        try:
            old_price_element = driver.find_element(By.XPATH, "//div[@class='CPriceBox-module_price__bYk-c']")
            old_price = old_price_element.text.strip()
        except NoSuchElementException:
            old_price = "-"
        # Extract description
        description_element = driver.find_element(By.XPATH, "//div[@class='ProductDescriptionTab_productDescriptionTab__CGdg7']")
...
for index, url in enumerate(urls):
    ...
    grid_class = 'PLPProductListing_PLPCardsWrapper__POow2'
    
    # Try locating all grids on the page with the given class
    try:
        grids = driver.find_elements(By.CLASS_NAME, 'PLPProductListing_PLPCardParent__GC2qb')
...
df.to_csv('sok_elements_info.csv', index=False, encoding='utf-8-sig')
